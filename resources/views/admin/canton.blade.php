    ...
    <div class="row justify-content-center">
        <div class="col-md-8 d-flex align-items-center">
            ...
            <div class="col-md-1 d-flex justify-content-center">
                <a class="btn btn-md btn-primary" href="{{ env('APP_URL') }}/agregarcanton" role="button">Nuevo</a>
            </div>
        </div>
            ...
            @if(count($cantones)>0)
                <table class="table table-secondary table-striped">
                    ...
                    <tbody>
                        @foreach ($cantones as $canton)
                            <tr>
                                <td>{{ $loop->index+1 }}</td>
                                <th>{{ $canton->nombre_canton }}</th>
                                <th>{{ $canton->nombre_provincia }}</th>
                                <td class="d-flex justify-content-center">
                                    <a class="me-2 text-success" href="{{ env('APP_URL') }}/editarcanton/{{ $canton->id_canton }}"><i class="fas fa-edit"></i></a>
                                    <div class="pointer ms-2 text-danger" data-bs-toggle="modal" data-bs-target="#modalEliminar" onclick="preparar_eliminacion('{{ $canton->id_canton }}', '{{ $canton->nombre_canton }}')"><i class="fas fa-trash-alt"></i></div>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                ...
                <nav>
                    <ul class="pagination">
                        ...
                        @else
                            <li class="page-item">
                                <a class="page-link" href="{{ env('APP_URL') }}/cantones/1">&lt;&lt;</a>
                            </li>
                            <li class="page-item">
                                <a class="page-link" href="{{ env('APP_URL') }}/cantones/{{ $pagina-1 }}">&lt;</a>
                            </li>
                        @endif
                        @for ($i = $inicioPaginacion; $i <= $finalPaginacion; $i++)
                            ...
                            @else
                                <li class="page-item"><a class="page-link" href="{{ env('APP_URL') }}/cantones/{{ $i }}">{{ $i }}</a></li>
                            @endif
                        @endfor
                        ...
                        @else
                            ...
                            <li class="page-item">
                                <a class="page-link" href="{{ env('APP_URL') }}/cantones/{{ $totalPaginas }}">&gt;&gt;</a>
                            </li>
                        @endif
                    </ul>
                </nav>
            @else
                <h3>No existen cantones registrados.</h3>
            @endif
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-danger" id="tituloeliminar">Eliminar Cantón: </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <p id="textoeliminar">¿Seguro que desea eliminar el cantón ?</p>
...
<script type="text/javascript">
    ...
    function preparar_eliminacion(id, nombre){
        document.getElementById("tituloeliminar").innerHTML="Eliminar Cantón: " + nombre;
        document.getElementById("textoeliminar").innerHTML="¿Seguro que desea eliminar el cantón " + nombre + " ?";
        document.getElementById("ideliminar").value=id;
    }
    function eliminar(){
        document.getElementById("btneliminar").disabled="disabled";
        let id=document.getElementById("ideliminar").value;
        axios.post(server+'/api/eliminarcanton', {
            id: id
        })
