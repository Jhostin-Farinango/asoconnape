    ...
    <div class="row justify-content-center">
        <div class="col-md-8 d-flex align-items-center">
            ...
            <div class="col-md-1 d-flex justify-content-center">
                <a class="btn btn-md btn-primary" href="{{ env('APP_URL') }}/agregarfpago" role="button">Nuevo</a>
            </div>
        </div>
            ...
            @if(count($formapagos)>0)
                <table class="table table-secondary table-striped">
                    ...
                    <tbody>
                        @foreach ($formapagos as $formapago)
                            <tr>
                                <td>{{ $loop->index+1 }}</td>
                                <th>{{ $formapago->nombre_forma_pago }}</th>
                                <td class="d-flex justify-content-center">
                                    <a class="me-2 text-success" href="{{ env('APP_URL') }}/editarfpago/{{ $formapago->id_forma_pago }}"><i class="fas fa-edit"></i></a>
                                    <div class="pointer ms-2 text-danger" data-bs-toggle="modal" data-bs-target="#modalEliminar" onclick="preparar_eliminacion('{{ $formapago->id_forma_pago }}', '{{ $formapago->nombre_forma_pago }}')"><i class="fas fa-trash-alt"></i></div>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                ...
                <nav>
                    <ul class="pagination">
                        ...
                        @else
                            <li class="page-item">
                                <a class="page-link" href="{{ env('APP_URL') }}/fpagos/1">&lt;&lt;</a>
                            </li>
                            <li class="page-item">
                                <a class="page-link" href="{{ env('APP_URL') }}/fpagos/{{ $pagina-1 }}">&lt;</a>
                            </li>
                        @endif
                        @for ($i = $inicioPaginacion; $i <= $finalPaginacion; $i++)
                            ...
                            @else
                                <li class="page-item"><a class="page-link" href="{{ env('APP_URL') }}/fpagos/{{ $i }}">{{ $i }}</a></li>
                            @endif
                        @endfor
                        ...
                        @else
                            ...
                            <li class="page-item">
                                <a class="page-link" href="{{ env('APP_URL') }}/fpagos/{{ $totalPaginas }}">&gt;&gt;</a>
                            </li>
                        @endif
                    </ul>
                </nav>
            @else
                <h3>No existen formas de pago registradas.</h3>
            @endif
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-danger" id="tituloeliminar">Eliminar Forma de Pago: </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <p id="textoeliminar">¿Seguro que desea eliminar la forma de pago ?</p>
...
<script type="text/javascript">
    ...
    function preparar_eliminacion(id, nombre){
        document.getElementById("tituloeliminar").innerHTML="Eliminar Forma de Pago: " + nombre;
        document.getElementById("textoeliminar").innerHTML="¿Seguro que desea eliminar la forma de pago " + nombre + " ?";
        document.getElementById("ideliminar").value=id;
    }
    function eliminar(){
        document.getElementById("btneliminar").disabled="disabled";
        let id=document.getElementById("ideliminar").value;
        axios.post(server+'/api/eliminarfpago', {
            id: id
        })
        .then(function (response) {
            ...
            if(respuesta.respuesta){
                alerta("success");
                setTimeout(function(){
                    window.location.href=server+"/fpagos";
                }, 3000);
            }
            else{
                alerta("danger");
