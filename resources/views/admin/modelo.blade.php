    ...
    <div class="row justify-content-center">
        <div class="col-md-8 d-flex align-items-center">
            ...
            <div class="col-md-1 d-flex justify-content-center">
                <a class="btn btn-md btn-primary" href="{{ env('APP_URL') }}/agregarmodelo" role="button">Nuevo</a>
            </div>
        </div>
            ...
            @if(count($modelos)>0)
                <table class="table table-secondary table-striped">
                    ...
                    <tbody>
                        @foreach ($modelos as $modelo)
                            <tr>
                                <td>{{ $loop->index+1 }}</td>
                                <th>{{ $modelo->nombre_modelo }}</th>
                                <th>{{ $modelo->nombre_marca }}</th>
                                <td class="d-flex justify-content-center">
                                    <a class="me-2 text-success" href="{{ env('APP_URL') }}/editarmodelo/{{ $modelo->id_modelo }}"><i class="fas fa-edit"></i></a>
                                    <div class="pointer ms-2 text-danger" data-bs-toggle="modal" data-bs-target="#modalEliminar" onclick="preparar_eliminacion('{{ $modelo->id_modelo }}', '{{ $modelo->nombre_modelo }}')"><i class="fas fa-trash-alt"></i></div>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                ...
                <nav>
                    <ul class="pagination">
                        ...
                        @else
                            <li class="page-item">
                                <a class="page-link" href="{{ env('APP_URL') }}/modelos/1">&lt;&lt;</a>
                            </li>
                            <li class="page-item">
                                <a class="page-link" href="{{ env('APP_URL') }}/modelos/{{ $pagina-1 }}">&lt;</a>
                            </li>
                        @endif
                        @for ($i = $inicioPaginacion; $i <= $finalPaginacion; $i++)
                            ...
                            @else
                                <li class="page-item"><a class="page-link" href="{{ env('APP_URL') }}/modelos/{{ $i }}">{{ $i }}</a></li>
                            @endif
                        @endfor
                        ...
                        @else
                            ...
                            <li class="page-item">
                                <a class="page-link" href="{{ env('APP_URL') }}/modelos/{{ $totalPaginas }}">&gt;&gt;</a>
                            </li>
                        @endif
                    </ul>
                </nav>
            @else
                <h3>No existen modelos registrados.</h3>
            @endif
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-danger" id="tituloeliminar">Eliminar Modelo: </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <p id="textoeliminar">¿Seguro que desea eliminar el modelo ?</p>
...
<script type="text/javascript">
    ...
    function preparar_eliminacion(id, nombre){
        document.getElementById("tituloeliminar").innerHTML="Eliminar Modelo: " + nombre;
        document.getElementById("textoeliminar").innerHTML="¿Seguro que desea eliminar el modelo " + nombre + " ?";
        document.getElementById("ideliminar").value=id;
    }
    function eliminar(){
        document.getElementById("btneliminar").disabled="disabled";
        let id=document.getElementById("ideliminar").value;
        axios.post(server+'/api/eliminarmodelo', {
            id: id
        })
