    ...
    <div class="row justify-content-center">
        <div class="col-md-8 d-flex align-items-center">
            ...
            <div class="col-md-1 d-flex justify-content-center">
                <a class="btn btn-md btn-primary" href="{{ env('APP_URL') }}/agregarmodulo" role="button">Nuevo</a>
            </div>
        </div>
            ...
            @if(count($modulos)>0)
                <table class="table table-secondary table-striped">
                    ...
                    <tbody>
                        @foreach ($modulos as $modulo)
                            <tr>
                                <td>{{ $loop->index+1 }}</td>
                                <th>{{ $modulo->nombre_modulo }}</th>
                                <td class="d-flex justify-content-center">
                                    <a class="me-2 text-success" href="{{ env('APP_URL') }}/editarmodulo/{{ $modulo->id_modulo }}"><i class="fas fa-edit"></i></a>
                                    <div class="pointer ms-2 text-danger" data-bs-toggle="modal" data-bs-target="#modalEliminar" onclick="preparar_eliminacion('{{ $modulo->id_modulo }}', '{{ $modulo->nombre_modulo }}')"><i class="fas fa-trash-alt"></i></div>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                ...
                <nav>
                    <ul class="pagination">
                        ...
                        @else
                            <li class="page-item">
                                <a class="page-link" href="{{ env('APP_URL') }}/modulos/1">&lt;&lt;</a>
                            </li>
                            <li class="page-item">
                                <a class="page-link" href="{{ env('APP_URL') }}/modulos/{{ $pagina-1 }}">&lt;</a>
                            </li>
                        @endif
                        @for ($i = $inicioPaginacion; $i <= $finalPaginacion; $i++)
                            ...
                            @else
                                <li class="page-item"><a class="page-link" href="{{ env('APP_URL') }}/modulos/{{ $i }}">{{ $i }}</a></li>
                            @endif
                        @endfor
                        ...
                        @else
                            ...
                            <li class="page-item">
                                <a class="page-link" href="{{ env('APP_URL') }}/modulos/{{ $totalPaginas }}">&gt;&gt;</a>
                            </li>
                        @endif
                    </ul>
                </nav>
            @else
                <h3>No existen módulos registrados.</h3>
            @endif
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-danger" id="tituloeliminar">Eliminar Módulo: </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <p id="textoeliminar">¿Seguro que desea eliminar el módulo ?</p>
...
<script type="text/javascript">
    ...
    function preparar_eliminacion(id, nombre){
        document.getElementById("tituloeliminar").innerHTML="Eliminar Módulo: " + nombre;
        document.getElementById("textoeliminar").innerHTML="¿Seguro que desea eliminar el módulo " + nombre + " ?";
        document.getElementById("ideliminar").value=id;
    }
    function eliminar(){
        document.getElementById("btneliminar").disabled="disabled";
        let id=document.getElementById("ideliminar").value;
        axios.post(server+'/api/eliminarmodulo', {
            id: id
        })
        .then(function (response) {
            ...
            if(respuesta.respuesta){
                alerta("success");
                setTimeout(function(){
                    window.location.href=server+"/modulos";
                }, 3000);
            }
            else{
                alerta("danger");
